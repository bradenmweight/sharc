...
subroutine write_Dmatrix(traj,ctrl)
  ...
  write(u_den,'(i5,1x,10(e23.12e3,1x))')traj%step,(histoF(i),&
          i=1,ctrl%nstates)

  !output action matrix
  write(u_action,'(i5,1x,25(e23.12e3,1x))')traj%step,((ActionMat(i,j),&
        j=1,ctrl%nstates),i=1,ctrl%nstates)

  !output G matrix elements for debugging
  write(u_gma,'(i5,1x,300(f16.8,1x))')traj%step,((traj%grad_ad(i,j),j=1,3),i=1,ctrl%natom)
 
  !output mapping variables for debugging
  write(u_map,'(i5,1x,20(f16.8,1x))')traj%step,qF,pF
  
  !output tot energy
  write(u_en,'(i5,1x,20(f16.8,1x))')traj%step,traj%Ekin,traj%Epot_qd,traj%Etot_qd
  
  !output overlap matrix
  if(traj%step==0)return
  write(u_over,'(i7,50(e23.12e3,1x))')traj%step,((traj%overlaps_ss(i,j),&
        j=1,ctrl%nstates),i=1,ctrl%nstates)
