...
! =================================================================== !
  ...
  type(ctrl_type) :: ctrl
  character*255 :: filename, windowfunc
  character*8000 :: geomfilename, line, windowfunctiontype, gammafunctiontype
  ...
  ! log file, list file, data file and geometry file
    ...
    ! make new files if no restart
      ...
      !action variables
      open(unit=u_action, file='action.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "action.dat"'
        stop 
      endif
      ...
      write(u_gma,*)"#Only print out G elements for debugging if printlevel>2"
      open(unit=u_map, file='map.dat', status='replace', position='rewind',iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "map.dat"'
        stop
      endif
      write(u_map,*)"# step, qF, pF if printlevel>2"
      open(unit=u_en, file='E.dat', status='replace', position='rewind',iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "E.dat"'
        stop
      endif
      !Wanghuai
    endif
  ...
  ! =====================================================
    ! BRADEN CHANGED HERE WINDOW FUNCTION AS windowfunc
    ! Look up window type
    line=get_value_from_key('windowfunc',io)
    write(*,*) "IO = ", io
    write(*,*) "WindowFunc written in input:", line
    if (io==0) then
      ! extract string within quotes (can contain spaces)
      call get_quoted(line,windowfunctiontype)
      ctrl%windowfunc=trim(windowfunctiontype)
      if (ctrl%windowfunc .ne. "rectangle" .and. ctrl%windowfunc .ne. "triangle" .and. ctrl%windowfunc .ne. "pyramidal") then
        write(*,*) "Not acceptable input to 'windowfunc' keyword:", line
        write(*,*) "Options: 'rectangle', 'triangle' (only for NSTATES = 2), 'pyramidal' "
        stop 1
      endif
    else
      ! Default windowfunc
      !windowfunc='rectangle'
      ctrl%windowfunc='TEST_WINDOWFUNC' ! Choose nothing until we know it works okay.
      write(*,*) "No window function specified using 'windowfunc' keyword." 
      write(*,*) "Options: 'rectangle', 'triangle (only for NSTATES = 2)', 'pyramidal' "
      write(*,*) "Default: Window set to,", ctrl%windowfunc
      stop 1
    endif

  ! =====================================================
    ! BRADEN CHANGED HERE GAMMA FUNCTION AS gammafunc
    ! Look up gamma function type
    line=get_value_from_key('adjustedgamma',io)
    write(*,*) "IO = ", io
    write(*,*) "Gammafunc written in input:", line
    if (io==0) then
      ! extract string within quotes (can contain spaces)
      call get_quoted(line,gammafunctiontype)
      ctrl%gammfunc=trim(gammafunctiontype)
      if ( ctrl%gammfunc .ne. "yes" .and. ctrl%gammfunc .ne. "no" ) then
        write(*,*) "Not acceptable input to adjustedGamma keyword:", line
        write(*,*) "Options: 'yes' or 'no' "
        stop 1
      endif
    else
      ! Default gammfunc
      !gammfunc='no'
      ctrl%gammfunc='TEST_GAMMAFUNC' ! Choose nothing until we know it works okay.
      write(*,*) "No gamma function specified using 'adjustedGamma' keyword."
      write(*,*) "Options: 'yes' or 'no' "
      stop 1
    endif
