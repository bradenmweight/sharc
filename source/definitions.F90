...
!Zhou
  ...
  real*8::Etot_qd  !total energy in QD basis
  real*8::Epot_qd  !potential energy in QD basis
!Wanghuai
...
!   real*8 :: diagonalize_degeneracy_diff=1.d-9     ! Energy difference threshold for treating states as degenerate                                in hartree
  ...
  character(len=20) :: windowfunc ! Window Fuction for Population ~ Braden Weight ~ 11/13/2020
  character(len=20) :: gammfunc ! Choose whether to use gamm-corrected or not (Default = not corrected) ~ Braden Weight ~ 11/13/2020
...
real*8,allocatable :: qF(:),pF(:)
real*8,allocatable :: qFd(:),pFd(:) !diabatic basis
real*8,allocatable :: ZPE(:)
real*8,allocatable :: ek(:)
real*8::qF0,pF0,qB0,pB0
real*8,allocatable::acti(:),angle(:),ActionMat(:,:)
complex*16,allocatable::Dmatrix(:,:)
integer,parameter::u_den=18
integer,parameter::u_over=19
integer,parameter::u_action=20
integer,parameter::u_map=21  !for mapping variables
integer,parameter::u_gma=22  !for G matrix
integer,parameter::u_en=23
real*8::gamm                 !constant used in SQC
real*8,allocatable::histoF(:) !histogram 
!...Wanghuai
